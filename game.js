...
const cellNeighbours = (rowCoordinate, columnCoordinate, currentBoard) => {
  ...
  coordModifiers.forEach(modifier => {
    ...
    // add one to the live neighbours count if the cell is alive and is on the game board
...
const gameMenu = () => {
  rl.question('To start the game press 1. To exit press 2. \n', (answer) => {
    if (answer === '1') { 
      let newGameBoard = seedBoard() // create a new game board with random cell values
      runGame(newGameBoard) // start a new game with this board
    }
    else if (answer === '2') {
      console.log("Goodbye!")
      return rl.close()
